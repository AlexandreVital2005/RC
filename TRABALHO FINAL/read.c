...
case S_BCC1_OK:
    if (Cread == C_I0 || Cread == C_I1) {
        ...
        if (buf == FLAG) {
            ...
            data[data_idx++] = buf;   // <- guardar o primeiro byte de dados
            state = S_DATA;
        }
    }
    else {
        if (buf == FLAG) {
            ...
            else if (Cread == C_UA && closing_phase && Aread == A_RX) {
    ...
    break;
            ...
            case S_DATA:
                if (buf == FLAG) {
                    ...
                    printf("Primeiros bytes destuffed: ");
                    for (int j = 0; j < 12 && j < payload_len; j++) {
                        printf("0x%02X ", destuffed[j]);
                    }
                    printf("\n");
