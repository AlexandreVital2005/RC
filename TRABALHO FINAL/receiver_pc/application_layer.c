...
// Transforma a informação num numero longo (0x01 0xFF = 1 * 256 + 255)
...
int parse_control_packet(const unsigned char *packet, int packet_len,char *filename_out,int filename_max,long *file_size_out)
{
    ...
    if (packet[0] != APP_START && packet[0] != APP_END){ 
        return -1;
    }
...
int handle_app_packet(const unsigned char *packet,int packet_len,AppReceiverContext *ctx)
{
    ...
    // Lê nome e tamanho, guarda, e abre ficheiro
    ...
    // Lê tamanho dos dados, escreve no ficheiro e soma ao total
    if (control == APP_DATA) {
        if (packet_len < 3) {
    ...
    // Confirma nome e tamanho, fecha ficheiro e marca como concluido
    if (control == APP_END) {
        long fsize = 0;
        char fname[256] = {0};
